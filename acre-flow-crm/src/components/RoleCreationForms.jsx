...
const RoleCreationForms = ({ userRole, formType }) => {
  ...
  const formConfigs = {
    'create-admin': {
      ...
      role: 'head-admin',
      departments: ['Sales', 'Marketing', 'Operations', 'Customer Success', 'HR', 'Finance'],
      permissions: ['manage_team_leaders', 'assign_leads', 'view_reports', 'create_tickets', 'manage_users', 'audit_logs']
    },
    'create-leader': {
      title: 'Create Team Leader',
      role: 'team-leader',
      departments: ['Sales Team A', 'Sales Team B', 'Marketing Team', 'Support Team', 'Product Team'],
      permissions: ['manage_employees', 'assign_leads', 'create_tickets', 'view_team_reports', 'manage_team_leads']
    },
    'create-employee': {
      title: 'Add Employee',
      role: 'employee',
      departments: ['Inside Sales', 'Field Sales', 'Customer Support', 'Marketing Assistant', 'Product Support'],
      permissions: ['view_assigned_leads', 'update_lead_status', 'manage_tickets', 'log_activities']
    }
  };
    ...
    const fetchReportingToOptions = async () => {
      ...
      if (roleToFetch) {
        try {
          ...
          const response = await fetch(`http://localhost:5001/api/users?role=${roleToFetch}`, {
            headers: {
      ...
      <div className="role-form-card">
        ...
        {userRole === 'super-admin' && formType === 'create-admin' && (
          <div className="info-box">
            ...
            <p><strong>Super Admin Action:</strong> You are creating a Head Admin user. This role has broad management capabilities.</p>
          </div>
        )}
