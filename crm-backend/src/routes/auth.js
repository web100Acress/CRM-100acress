...
const { getValidToken } = require('../services/tokenRefreshService');
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // ✅ If user is Boss, ensure we have a valid 100acress service token
    if (user.role?.toLowerCase() === 'boss') {
      console.log('🔑 Boss logged in, checking 100acress service token...');
      getValidToken().catch(err => console.error('❌ Error refreshing service token on Boss login:', err.message));
    }
